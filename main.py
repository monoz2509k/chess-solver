...
def show_menu(window, clock):
    ...
    while running:
        ...
        for i, d in enumerate(depth_options):
            ...
            desc_surf = font_small.render(d["desc"], True, C_GOLD_DIM if is_sel else C_TEXT_DIM)
    ...
    def redraw():
        ...
        for line in board.board:
            ...
            for pos, row in line:
                ...
                if (y, x) in moves:
                    pygame.draw.circle(window, (220, 80, 80), (pos[0]+30, pos[1]+35), 6)
                x += 1
            y += 1

        if board.promotion:
            if board.turn != 0: window.blit(board.white_promotion, (20, 20))
            else:               window.blit(board.black_promotion, (20, 20))

        game_over = board.checkmate or board.stalemate or board.draw
        if board.checkmate:      window.blit(checkmate_msg, (8, 4))
        elif board.stalemate:    window.blit(stalemate_msg, (8, 4))
        elif board.draw:         window.blit(draw_msg,      (8, 4))
        elif board.check > 0:    window.blit(check_msg,     (8, 4))
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if board.promotion and human_team >= 0:
                    ...
                    mapping = {
                        pygame.K_1: Queen,  pygame.K_KP_1: Queen,
                        pygame.K_2: Rook,   pygame.K_KP_2: Rook,
                        pygame.K_3: Bishop, pygame.K_KP_3: Bishop,
                        pygame.K_4: Knight, pygame.K_KP_4: Knight,
                    }
                    cls = mapping.get(event.key)
                    if cls:
                        extra = (-1,) if cls == Rook else ()
                        board.board[promotion_y][promotion_x][1] = cls(team, f"{color}_{cls.__name__.lower()}.png", *extra)
                    else:
